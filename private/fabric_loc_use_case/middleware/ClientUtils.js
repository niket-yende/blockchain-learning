...
module.exports.KVS = path.join(tempdir, "hfc");
  ...
  return client
    ...
    .then(user => {
      return new Promise((resolve, reject) => {
        ...
        logger.info(util.format("11"));

        // need to enroll it with CA server
        var cop = new copService(
          caUrl,
          tlsOptions,
          ORGS[userOrg].ca.name,
          cryptoSuite
        );
        logger.info(util.format("12"));
        return cop
          .enroll({
...
function registerAndEnrollUser(client, cop, admin, username, userOrg) {
  return new Promise((resolve, reject) => {
    ...
    return cop
      ...
      .catch(err => {
        console.log(
          "Failed to register user. Error: " + err.stack ? err.stack : err
        );
        throw err;
      })
      .then(enrollment => {
        ...
        return enrollUser.setEnrollment(
          enrollment.key,
          enrollment.certificate,
          ORGS[userOrg].mspid
        );
...
function getAdmin(client, userOrg) {
  var keyPath = path.join(
    __dirname,
    util.format(
      Constants.networkLocation +
        "/crypto-config/peerOrganizations/%s.trade.com/users/Admin@%s.trade.com/msp/keystore",
      userOrg,
      userOrg
    )
  );
  ...
  var certPath = path.join(
    ...
    util.format(
      Constants.networkLocation +
        "/crypto-config/peerOrganizations/%s.trade.com/users/Admin@%s.trade.com/msp/signcerts",
      userOrg,
      userOrg
    )
  );
...
function getOrdererAdmin(client) {
  var keyPath = path.join(
    __dirname,
    Constants.networkLocation +
      "/crypto-config/ordererOrganizations/trade.com/users/Admin@trade.com/msp/keystore"
  );
  var keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
  var certPath = path.join(
    __dirname,
    Constants.networkLocation +
      "/crypto-config/ordererOrganizations/trade.com/users/Admin@trade.com/msp/signcerts"
  );
  var certPEM = readAllFiles(certPath)[0];
...
function readFile(path) {
  return new Promise((resolve, reject) => {
    fs.readFile(path, (err, data) => {
      if (!!err)
        reject(
          new Error("Failed to read file " + path + " due to error: " + err)
        );
      else resolve(data);
    });
  });
...
function getClientUser(userOrg, username, password) {
  ...
  logger.info(util.format("2"));
  var orgName = ORGS[userOrg].name;
  var client = new Client();

  var cryptoSuite = Client.newCryptoSuite();
  cryptoSuite.setCryptoKeyStore(
    Client.newCryptoKeyStore({ path: module.exports.storePathForOrg(orgName) })
  );
  client.setCryptoSuite(cryptoSuite);
  logger.info(util.format("3"));
